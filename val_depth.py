...
def save_one_json(predn, jdict, path, class_map):
    ...
    box[:, :2] -= box[:, 2:4] / 2  # xy center to top-left corner
...
def run(
    ...
    loss = torch.zeros(4, device=device) # 4 losses for depth
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        # NMS
        targets[:, 2:6] *= torch.tensor((width, height, width, height), device=device)  # to pixels, specify 6 for depth
        lb = [targets[targets[:, 0] == i, 1:] for i in range(nb)] if save_hybrid else []  # for autolabelling

        with dt[2]:
            preds = non_max_suppression_depth(preds,
                                            conf_thres,
                                            iou_thres,
                                            labels=lb,
                                            multi_label=True,
